(function ($) {
  ...
  // --- Parse a single work page for metadata ---
  function parseWorkPage(html) {
    ...
    var category = $doc.find(".category .tag, dd.category.tags a.tag").map(function () {
      return $(this).text();
    }).get().join(", ");
  ...
  // --- Collect items from one collection-items page (current or fetched) ---
  function collectItemsFromDoc($doc) {
    $doc.find("li.collection.item.picture.blurb.group").each(function () {
      ...
      var unrevealed = $li
        .find("input[type=checkbox][name$='[unrevealed]']")
        .is(":checked")
        ? "unrevealed"
        : "revealed";
  ...
  // --- Recursively process all pages starting from "nextUrl" ---
  ...
  // 1) Collect items from current page
  var $current = $(document);
  collectItemsFromDoc($current);
  var firstNext = getNextHref($current);

  // 2) Process remaining pages, then fetch per-work data, then build CSV
